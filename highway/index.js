...
var ref$, w, h, main;
ref$ = [600, 600], w = ref$[0], h = ref$[1];
main = function($scope, $http){
  $scope.force = d3.layout.force().gravity(0.05).charge(function(it){
    return it.name.length * -20 - 20;
  });
  $scope.start = null;
  $scope.end = "";
  $scope.miles = 0;
  ...
  return $http.get('no1.json').success(function(data){
    ...
    z4$.append('text').attr('class', 'text-bg').attr('dy', '15px').text(function(it){
      return it.name;
    });
    z4$.append('text').attr('class', 'text-fg').attr('dy', '15px').text(function(it){
      return it.name;
    });
